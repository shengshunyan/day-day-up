...
1. 迭代器
    - 接口：(补充：Iterable即@@iterator属性为Iterator生成函数)
    - next()迭代
    ```JavaScript
...
2. 生成器
    ...
    ```
    - 迭代器控制：next()方法双向传递值
    - 提前完成：生成器上附着地迭代器支持可选return()和throw()方法，可终止一个暂停的生成器
    ```JavaScript
    function *foo() {
        try {
            yield 1;
...
4. 类
    ...
    - extends和super
        ...
        a.first(); // 1
        ```
    - new.target: 总是指向new实际直接调用的构造器；
    - static：当子类Bar从弗雷Foo拓展时，不仅Bar.prototype是[[prototype]]链接到Foo.prototype，而且Bar()也[[prototype]]连接到Foo()；
    ```JavaScript
...
1. Array
    - 静态函数Array.of(..): 解决Array构造器的一个陷阱，就是只传入一个参数，并且这个参数是数字的话，那么就会构造一个空数组，其length为这个数字；
    ```JavaScript
    var a = Array(3);
    a.length; // 3
    a[0]; // undefined
...
2. Object
    ...
    ```
    - 静态函数Object.getOwnPropertySymbols(..): 获取对象中的所有符号属性；
    - 静态函数Object.setPrototypeOf(..): 用于行为委托；
    - 静态函数Object.assign(..): 用于把一个对象的属性复制/混合到另一个对象中；
    - Object.create(..) --ES5
3. Math: 增加了一些数学函数；
4. Number
    - 静态属性: Number.EPSILON, MAX_SAFE_INITEGER, MIN_SAFE_INTEGER;
    - 静态函数Number.isNaN(..)
    - 静态函数Number.isFinite(..)
    - 静态函数Number.isInteger(..)
    - 静态函数Number.isSafeInteger(..)
5. String
    - Unicode函数: String.fromCodePoint(..), String#codePointAt(..), String#normalize(..);
    - 静态函数String.raw(..)
    ```JavaScript
    var str = 'bc';
    ...
    ```
    - 字符串检查函数：startsWith(..), endsWith(..), includes(..);
...
1. 函数名称
    ...
    - 函数名称的推导；
2. 元属性
    - new.target: 构造器调用的内部使用new.target时，指向调用new的目标构造器；
3. 公开符号：JavaScript预先定义了一些内置符号；
    - Symbol.iterator: 对象的一个属性(方法)，构造一个迭代器来消耗这个对象；
    - Symbol.toStringTag, Symbol.hasInstance: 可以操作toString()和instanceof()函数的行为特性；
    - Symbol.species: 控制要生成新实例时，类的内置方法使用哪个构造器；
    - Symbol.toPrimitive: 控制对象为了某个操作(==或+)必须强制转换为一个原生类型值的时候的行为；
    - 正则表达式符号: Symbol.match, Symbol.replace, Symbol.search, Symbol.split, 控制对应的原型方法的行为；
    - Symbol.isConcatSpreadable: 指示如果把它传给一个数组的concat(..)是否将其展开；
4. 代理
    - 语法
    ```JavaScript
    var obj = { a: 1 },
    ...
    ```
    - 还有其他代理函数：get(), set(), deleteProperty(), apply()...等；
    - 可取消代理
    ```JavaScript
    var obj = { a: 1 },
        handlers = {
...
5. Reflect API
    ...
    ```
6. 属性顺序
    - (不可靠) Reflect.enumerate(..), Object.keys(..)和for..in，可观察的顺序是相同的，但是不再必须与Reflect.ownKeys(..)相同，依赖具体的实现；
    - (可靠) ES6中，属性的列出顺序由[[OwnPropertyKeys]]算法定义，这个算法只对Reflect.ownKeys(..), Object.getOwnPropertyNames(..), Object.getOwnPropertySymbols(..)有保证；
    - [[OwnPropertyKeys]]算法属性顺序：
...
7. 特性测试
    ...
    ```
    - 工具：FeatureTests.io进行特性测试；
8. 尾递归调用
    - 尾调用：这个优化只在strict模式下应用；函数调用处于代码路径上最后发生的一件事(即return函数调用)；
    - 递归重写
    ```JavaScript
    'use strict';
    function foo(x) {
    ...
    foo(123456); // 3810376848.5
    ```
    - TCO特性测试：决定采用 尾调用优化 还是 非尾调用优化(展开递归)
    ```JavaScript
    'use strict';
    try {
        (function foo(x) {
...
7. WebAssembly(WASM): 更好的性能
    - WASM是一种代码的高度压缩AST二进制表示格式，可以直接向JavaScript引擎发出指令，像C或者C++这样的语言可以直接编译为WASM；
    - 之前：其他语言 -> ASM.js -> JS引擎； 现在：其他语言 -> WebAssembly -> JS引擎；
    - JavaScript是未来不太可能转化为WASM的语言之一，但是JavaScript和WASM代码可以最大程度的交互，就像现在的模块交互一样；
