...
1. CommonJS
    ...
    - CommonJS规范为JavaScript指明了一条非常棒的道路，规范涵盖了模块、二进制、Buffer、字符集编码、I/O流、进程环境、文件系统、套接字、单元测试、Web服务器网关接口、包管理器等；
    - CommonJS的模块规范：模块引用、模块定义、模块标识(传递给require()方法的参数，必须是小驼峰命名)；
<!-- more -->

2. Node的模块实现
    - Node中引入模块，需经历三个步骤：路径分析、文件定位、编译执行；
    - Node中，模块分为两类：核心模块、文件模块；
    - 优先从缓存加载：Node会缓存模块编译和执行后的对象；
    - 路径分析：
...
3. 核心模块：
    - JavaScript核心模块(lib目录下)的编译过程；
    - C/C++核心模块(src目录下)的编译过程；
    - 核心模块的引入流程；
    - 编写核心模块；
4. C/C++拓展模块
5. 模块调用栈
6. 包与NPM
    - 包的结构
    - 包描述文件(package.json)
    - 全局安装-g：-g是将一个包安装为全局可用的可执行命令；它根据包描述文件中的bin字段配置，将实际脚本链接到与Node可执行文件相同的路径下；
    - npm钩子命令：在install前后可监听函数；
    - 发布包(还有包的权限管理)；
    - 局域npm：搭建企业内部保密的npm仓库；
    - 高质量的npm包：良好的测试、良好的文档、良好的测试覆盖率、编码规范；
7. 前后端共用模块
    - 后端模块获取是磁盘读取，可以同步引入；但是前端模块是通过网络加载，需要支持异步引入；
    - 后端模块：CommonJS规范；
    - 前端模块：AMD、CMD
    ```JavaScript
    // AMD
...
1. 为什么要异步I/O:
    - 多线程：状态同步、死锁的问题；
    - 同步I/O：阻塞；
    - Node: 单线程、异步I/O，分配资源更高效；
2. 异步I/O的实现现状：(实际上非阻塞和异步是不一样的)
    - 非阻塞I/O：为获取完整数据，应用程序需要重复调用I/O操作来确认是否完成，这种技术叫轮询；
    - 异步I/O的实现：一个主线程来做计算处理，其他线程进行I/O操作(阻塞或非阻塞)，通过线程的通信将I/O的数据进行传递，这就实现了异步I/O；
    - Node是单线程的，但仅仅是JavaScript执行在单线程中，内部完成I/O任务的另有线程池；
...
4. 非I/O的异步API：
    ...
    ```
5. 事件驱动与高性能服务器
    - 经典服务器模型：同步式、每进程/每请求、每线程/每请求(Apache)；
    - Node通过事件驱动的方式处理请求，无需为每个请求创建额外的对应线程；知名服务器Nginx，也摒弃了多线程的方式，采用了和Node相同的事件驱动；
...
2. 异步编程的优势与难点
    ...
    - 难点：
        ...
        5. 异步转同步；
3. 异步编程解决方案：
    - 事件发布/订阅模式：事件emitter对象；
    - Promise/Deferred模式：书本P90-91页有Promise的实现原理(为啥promise只会决议一次 shift);
    - 流程控制库：尾触发与Next、async、step、wind；
...
1. V8的垃圾回收机制与内存限制
    - Node基于V8构建，而V8的内存管理机制主要面向浏览器的应用场景，所以在内存使用方面做了限制，只能使用部分内存(64位系统下约为1.4G，32位系统下约为0.7G)；
    - V8的对象分配
...
3. 内存指标：
    ...
    - Node的内存构成主要由通过V8进行分配的部分和Node自行分配的部分，受V8的垃圾回收限制的主要是V8的堆内存，而Buffer对象制类的不同于其他对象，属于堆外内存，不受V8限制；
4. 内存泄漏：缓存、队列消费不及时、作用域未释放；
    - 慎将内存做缓存：如果利用JavaScript对象键值来缓存东西，则需要限定缓存对象的大小，防止内存无限增长；
    ```JavaScript
    ...
    ```
    - 缓存的解决方案：采用进程外的缓存，减少进程常驻对象的数量，进程之间还可以共享缓存；
        1. Redis
        2. Memcached
    - 关注队列状态：
...
```
5. Buffer的拼接：
    - 问题描述：Buffer的使用场景中，通常是一段一段的方式传输的，英文不会出现问题，但是遇到中文宽字符编码时(中文在utf-8下占3个字节)，就可能会出现中文乱码问题；
    ```JavaScript
    var fs = require('fs');
    ...
    ```
6. Buffer与性能：
    - Buffer在文件I/O和网络I/O中运用广泛，尤其在网络传输中，性能举足轻重；
    - 文件读取时，highWaterMark设置对大文件读取的性能有影响；
    - Buffer时二进制数据，字符串与Buffer之间存在编码关系；

### 第七章 网络编程

1. 构建TCP服务
    - TCP：传输控制协议，属于传输层协议，三次握手；
    - 创建TCP服务器端
    ```JavaScript
    var net = require('net');
    ...
    ```
    - TCP服务的事件；优化策略(Nagle算法: 小数据合并延迟发送);
2. 构建UDP服务
    - UDP：用户数据包协议，属于传输层协议，不是面向连接的；
    - 创建UDP服务器端
    ```JavaScript
    ...
    ```
3. 构建HTTP服务
    - HTTP报文：TCP的3次握手 -> 客户端向服务器端发送请求报文 -> 服务器端向客户端发送响应内容 -> 结束会话；
    - 创建HTTP服务器端
    ```JavaScript
...
4. WebSocket协议：客户端与服务器端只建立一个TCP连接，可以使用更少的连接；
    - WebSocket握手部分是由HTTP完成的，但是它是在TCP上定义独立的协议；相比HTTP，WebSocket更接近于传输层协议；
    - WebSocket握手：客户端通过HTTP发送请求报文，请求服务器端升级为WebSocket；服务器端告知客户端正在更换协议；
    - WebSocket数据传输：客户端和服务器端都可以接受和发送消息；为了安全考虑，客户端需要对发送的数据帧进行掩码处理，而服务器发送到客户端的数据则无须做掩码处理；
5. 网络服务与安全
    - TLS/SSL：是一个公钥/私钥的结构；SSL组为一种安全协议，它在传输层上提供对网络连接加密的功能；数字证书来进行认证；
    - HTTPS：就是工作在TLS/SSL上的HTTP；
...
1. 基础功能
    ...
    - Basic认证：客户端与服务器端进行请求时，允许通过用户名和密码实现的一种身份认证的方式(不够安全)；
...
3. 路由解析：
    - 文件路径型：URL的路径与网站资源的路径一致；
    - MVC：路由解析，根据URL找到对应的控制器 -> 行为调用相关模型，操作数据 -> 操作数据结束后，渲染视图；
...
2. 多进程架构：面对单进程单线程对多核使用不足额问题，启动多进程为了充分利用CPU资源；理想状态下每个进程各自利用一个CPU，以此实现多核CPU地利用；
    - 创建子进程：spawn(), exec(), execFile(), fork();(事实上后面3钟方法都是spawn()地延伸应用)
    - 进程间通信：
    ...
    - 句柄传递：解决不同进程不能监听相同端口的问题；
3. 集群稳定之路：用child_process模块在单机上搭建Node集群；
    - 进程事件：error, exit, close, disconnect
    - 自动重启：监听子进程的exit事件来获知其退出的信息，然后重新启动一个工作进程来继续服务；
        1. 自杀信号：提前重启一个新进程；
        2. 限量重启；
    - 负载均衡：分清进程的CPU、I/O两部分的繁忙程度；
    - 状态共享：
        1. 第三方数据存储；
...
4. Cluster模块：进程管理，隐藏child_process的一些细节；
    ...
    ```
    - Cluster事件
    - cluster模块应用中，一个主进程只能管理一组工作进程；通过child_process操作子进程时更灵活；

### 第十章 测试

1. 单元测试：测试用例、测试覆盖率；
2. 性能测试：
    - 基准测试：统计多少时间内执行了多少次某个方法，一般会以次数作为参照，然后比较时间；(benchmark模块)
    - 压力测试：对网络接口做压力测试需要考查的几个指标有吞吐率、响应时间和并发数；最常用的工具是ab, siegn, http_load等；还有一个比较重要的指标是QPS(Requests per second)，它标识每秒能处理多少请求；
    - 启动多个进程，并没有显著提升QPS；
    - 测试数据与业务数据的转换：QPS = PV / 24h; (PV是日均访问量，时间可能还要小，因为一般集中在某段时间内)
...
2. 部署流程：
    ...
    - 部署操作
3. 性能：
    - 动静分离：Node处理静态文件的能力不算突出，所以让Node只处理动态请求即可；将静态资源都引导到专业的静态服务器上，如Nginx或者专业的CDN；
    - 启用缓存：第三方缓存Redis；
    - 多进程架构：cluster模块；
    - 读写分离：数据库的读操作远远高于写操作，为了提升性能，通常会经常数据库的读写分离；
4. 日志
5. 监控警报：
    - 监控：日志监控、响应时间、进程监控、磁盘监控、内存监控、CPU占用监控等；
    - 警报：邮件警报(nodemailer模块)，短信电话；
6. 稳定性：多机器，多机房，容灾备份；